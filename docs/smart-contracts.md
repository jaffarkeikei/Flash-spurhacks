...
This document details the Move smart contracts that power the Aptos Flash platform. These contracts handle the on-chain components of cross-border payments, including escrow, settlement, and gas sponsorship.

## Contract Overview

The Flash system consists of four primary smart contract modules:

1. **FlashModule**: Main module that handles payment creation, execution, and management
...
The FlashModule serves as the main entry point for creating and managing cross-border payments. It coordinates between the other modules to execute end-to-end payment flows.

```move
module flash::flash_module {
    use std::signer;
    use std::string::{String};
    use aptos_framework::coin::{Self, Coin};
    use aptos_framework::timestamp;
    use aptos_framework::account;
    use flash::escrow_module;
    use flash::gas_station_module;
    use flash::fee_module;
    ...
    public fun create_payment<CoinType>(
        ...
        let payment_store = borrow_global_mut<PaymentStore>(@flash);
    ...
    public fun execute_payment<CoinType>(
        ...
        assert!(operator_addr == @flash, ENOT_AUTHORIZED);
...
module flash::escrow_module {
    ...
    public fun create_escrow<CoinType>(
        ...
        if (!exists<EscrowStore<CoinType>>(@flash)) {
            move_to(@flash, EscrowStore<CoinType> {
                escrows: table::new(),
        ...
        let escrow_store = borrow_global_mut<EscrowStore<CoinType>>(@flash);
...
```move
module flash::gas_station_module {
    use std::signer;
    use std::string::{String};
    use std::vector;
    use aptos_framework::account;
    ...
    public fun init_module(admin: &signer) {
        ...
        assert!(admin_addr == @flash, ENOT_AUTHORIZED);
    ...
    public fun register_sponsor(
        ...
        let registry = borrow_global_mut<SponsorRegistry>(@flash);
    ...
    public fun sponsor_transaction(
        ...
        let registry = borrow_global<SponsorRegistry>(@flash);
...
module flash::fee_module {
    ...
    public fun calculate_fee<CoinType>(amount: u64): u64 acquires FeeParameters {
        let fee_params = borrow_global<FeeParameters>(@flash);
    ...
    public fun process_fee<CoinType>(
        ...
        if (!exists<FeeVault<CoinType>>(@flash)) {
            move_to(@flash, FeeVault<CoinType> {
                coins: coin::zero<CoinType>()
            });
        };
        ...
        let fee_vault = borrow_global_mut<FeeVault<CoinType>>(@flash);
    ...
    public fun update_fee_parameters(
        ...
        let fee_params = borrow_global_mut<FeeParameters>(@flash);
...
The gas sponsorship feature in Flash is a critical component for providing a seamless user experience, particularly for users who don't hold APT tokens or are new to the Aptos ecosystem.
...
2. The Flash backend service identifies the transaction as eligible for sponsorship
...
public entry fun create_sponsored_transaction(
    ...
    let txn_payload = flash_module::create_payment<USDC>(user_account, recipient, amount);
...
fun test_create_and_execute_payment() {
    ...
    // Create payment
    let payment_id = flash_module::create_payment<USDC>(
        &sender,
        @0xB,
        500
    );
    ...
    let status = flash_module::get_payment_status(payment_id);
    assert!(status == 0, 0); // STATUS_CREATED

    // Execute payment
    flash_module::execute_payment<USDC>(&admin, payment_id);

    // Verify payment was completed
    status = flash_module::get_payment_status(payment_id);
...
The Flash contracts will be deployed to the Aptos mainnet at the following addresses:
